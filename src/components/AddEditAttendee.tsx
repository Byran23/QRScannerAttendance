...
export default function AddEditAttendee({ onNavigate, editData }: AddEditAttendeeProps) {
  ...
  return (
    <div className="space-y-4">
      <button onClick={() => onNavigate('attendees')} className="flex items-center gap-2 text-gray-600 dark:text-slate-400 hover:text-gray-800 dark:hover:text-white transition-colors"><ArrowLeft size={18} /> Back to Attendees</button>
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-slate-800 transition-colors">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center text-blue-600 dark:text-blue-400"><User size={20} /></div>
          <div>
        ...
        {saved && <div className="mb-4 p-3 bg-green-50 dark:bg-green-950/50 border border-green-200 dark:border-green-800 rounded-xl text-green-700 dark:text-green-400 text-sm font-medium">✅ Attendee {isEdit ? 'updated' : 'added'} successfully!</div>}
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-slate-800 rounded-xl">
            <div className={`w-14 h-14 rounded-full ${previewBg} flex items-center justify-center text-white text-xl font-bold`}>{previewInitials}</div>
            <div>
              <p className="text-sm font-medium text-gray-700 dark:text-slate-300">{form.name.trim() || 'Enter a name'}</p>
              <p className="text-xs text-gray-400 dark:text-slate-500">Initials are generated automatically from the name</p>
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-4">
            <button type="submit" disabled={saved} className="bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white px-6 py-2.5 rounded-xl font-medium flex items-center gap-2 transition-all shadow-lg shadow-blue-200 dark:shadow-blue-900/30"><Save size={18} />{isEdit ? 'Update' : 'Add'} Attendee</button>
            <button type="button" onClick={() => onNavigate('attendees')} className="px-6 py-2.5 rounded-xl font-medium text-gray-600 dark:text-slate-300 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 transition-all">Cancel</button>
          </div>
        </form>
      </div>
    </div>
...
function InputField({ label, value, onChange, error, placeholder, type = 'text' }: { label: string; value: string; onChange: (v: string) => void; error?: string; placeholder?: string; type?: string }) {
  return (
    <div>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className={`w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 transition-all ${error ? 'border-orange-300 dark:border-orange-700 focus:ring-orange-500 bg-orange-50 dark:bg-orange-950/30' : 'border-gray-200 dark:border-slate-700 focus:ring-blue-500 bg-white dark:bg-slate-800'} dark:text-white dark:placeholder-slate-500`} />
