...
export default function QRScanner() {
  ...
  const startScanner = useCallback(async () => {
    ...
    try {
      ...
      // Adjusted qrbox size slightly up to fit larger viewport better
      await scanner.start({ facingMode: 'environment' }, { fps: 10, qrbox: { width: 280, height: 280 }, aspectRatio: 1.0 }, handleScan, () => {});
      setIsScanning(true);
    } catch (err: unknown) {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden transition-colors">
        <div className="relative">
          {/* Increased minimum height from 350px to 500px / h-[500px] */}
          <div id="qr-reader" className={`w-full ${isScanning ? 'min-h-[500px] sm:min-h-[550px]' : 'h-0 overflow-hidden'}`} />
          
          {!isScanning && !cameraError && !pendingScan && (
            <div className="flex flex-col items-center justify-center min-h-[450px] py-16 px-6">
              ...
              <h3 className="text-xl font-semibold text-gray-800 dark:text-white mb-2">Ready to Scan</h3>
              <p className="text-gray-500 dark:text-slate-400 text-sm text-center max-w-xs mb-8">Point your camera at an attendee's QR code to record attendance</p>
              <button onClick={startScanner} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3.5 rounded-xl font-medium flex items-center gap-2 transition-all shadow-lg shadow-blue-200 dark:shadow-blue-900/30 text-lg">
          ...
          {cameraError && (
            <div className="flex flex-col items-center justify-center min-h-[450px] py-16 px-6">
              ...
              <button onClick={startScanner} className="bg-orange-600 hover:bg-orange-700 text-white px-6 py-2.5 rounded-xl font-medium flex items-center gap-2 transition-all">
        ...
        {isScanning && !pendingScan && (
          <div className="p-4 border-t border-gray-100 dark:border-slate-800">
            <div className="flex items-center justify-between">
              ...
              <button onClick={stopScanner} className="bg-orange-100 dark:bg-orange-950/50 hover:bg-orange-200 dark:hover:bg-orange-900/50 text-orange-700 dark:text-orange-400 px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-2 transition-all">
